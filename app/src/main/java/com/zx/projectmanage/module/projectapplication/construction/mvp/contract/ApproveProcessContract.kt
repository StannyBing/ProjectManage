 ...
 * 功能：
 */
interface ApproveProcessContract {
    //对于经常使用的关于UI的方法可以定义到IView中,如显示隐藏进度条,和显示文字消息
    interface View : IView {
        fun getDataProcessResult(data: MutableList<ApproveProcessInfoBean>?)
    }
    ...
    interface Model : IModel {
        fun getProcessInfo(
            projectId: String,
            subProjectId: String
        ): Observable<MutableList<ApproveProcessInfoBean>>
    }

    //方法
    abstract class Presenter : BasePresenter<View, Model>() {
        abstract fun getProcessInfo(
            projectId: String,
            subProjectId: String
        )
    }
