 ...
 * 气泡布局
 */
public class MBubbleRelativeLayout extends RelativeLayout {
    ...
    public MBubbleRelativeLayout(Context context, @ColorRes int bgColor) {
        this(context, null, bgColor);
    }

    public MBubbleRelativeLayout(Context context, AttributeSet attrs, @ColorRes int bgColor) {
        super(context, attrs);
    ...
    private void init(final Context context, final AttributeSet attrs, @ColorRes int mColor) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.mbubble);

            try {
                PADDING = a.getDimensionPixelSize(R.styleable.mbubble_padding, PADDING);
                SHADOW_COLOR = a.getInt(R.styleable.mbubble_shadowColor, SHADOW_COLOR);
                LEG_HALF_BASE = a.getDimensionPixelSize(R.styleable.mbubble_halfBaseOfLeg, LEG_HALF_BASE);
                MIN_LEG_DISTANCE = PADDING + LEG_HALF_BASE;
                STROKE_WIDTH = a.getFloat(R.styleable.mbubble_stroke_width, STROKE_WIDTH);
                CORNER_RADIUS = a.getFloat(R.styleable.mbubble_corner_radius, CORNER_RADIUS);
            } finally {
                if (a != null) {
                    a.recycle();
                }
...
//        float[] radiusArray = {CORNER_RADIUS,CORNER_RADIUS,CORNER_RADIUS,CORNER_RADIUS,CORNER_RADIUS,CORNER_RADIUS,CORNER_RADIUS,CORNER_RADIUS};
    ...
    public int transColor(int color) {
        try {
            if (color > 0) {
